...
namespace Loader
{
    public partial class Form1 : MetroForm
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            var result = ping.Send("localhost");

            if (result.Status == System.Net.NetworkInformation.IPStatus.Success)
            {

                string updaterr = "aHR0cHM6Ly9lZmZleC54eXovYXV0aC92ZXJzaW9uLnR4dA==";

                byte[] dataa = Convert.FromBase64String(updaterr);
                string updatarr = Encoding.UTF8.GetString(dataa);

                WebRequest request = WebRequest.Create(updatarr);
            ...
            string updater = "aHR0cHM6Ly9lZmZleC54eXovYXV0aC9zdGF0dXMudHh0";

            byte[] data = Convert.FromBase64String(updater);
            string updatar = Encoding.UTF8.GetString(data);

            var webRequest = WebRequest.Create(updatar);
            ...
            using (var reader = new StreamReader(content))
            {
                ...
                if (status == "0")
                {
                    MessageBox.Show("Error: The cheat has been locked!", "Error!");
                    Application.Exit();
                }
                else if (status == "1")
                {
                ...
                {
                    MessageBox.Show("Error: The cheat is down for updates! ", "Error!");
                    Application.Exit();
                }
                else if (status == "3")
                {
                    MessageBox.Show("Error: The cheat is closed!", "Error!");
                    Application.Exit();
                }
            }
        ...
        {
            string updater = "aHR0cHM6Ly9lZmZleC54eXovYXV0aC92ZXJzaW9uLnR4dA==";
        ...
        private void timer2_Tick(object sender, EventArgs e)
        {
            ...
            string updater = "aHR0cHM6Ly9lZmZleC54eXovYXV0aC9sb2FkZXIuZXhl";

            byte[] data = Convert.FromBase64String(updater);
            string updatar = Encoding.UTF8.GetString(data);
            
            myWebClient.DownloadFile(updatar, Directory.GetCurrentDirectory() + "/" + finalString + ".exe");
